...
it('should handle hash in initial url', () => {
    const url = 'https://example.com/?model=new#key';
    const query = {
        view: 'dashboard',
        component: 'widget',
        users: ['1', '2'],
    ...
    const result = addQueryToUrl(url, query);
    expect(result).toEqual('https://example.com/?model=new&view=dashboard&component=widget&users=1&users=2&count=5&isActive=true#key');
});